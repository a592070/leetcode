...
func cloneGraph(node *Node) *Node {
	var memorizeNode = map[int]*Node{}
	return dfsCopy(memorizeNode, node)
}

func dfsCopy(memorizeNode map[int]*Node, node *Node) *Node {
	if node == nil {
		return nil
	}
	copied := &Node{
	...
	memorizeNode[copied.Val] = copied
	if len(node.Neighbors) > 0 {
		for i, neighbor := range node.Neighbors {
			if memorizeNode[neighbor.Val] != nil {
				copied.Neighbors[i] = memorizeNode[neighbor.Val]
				continue
			}
			copied.Neighbors[i] = dfsCopy(memorizeNode, neighbor)
		}
	}
	return copied
}
